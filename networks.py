...
class RSSM(nn.Module):
    ...
    def observe_stats(self, embed, action, is_first, state=None):
        swap = lambda x: x.permute([1, 0] + list(range(2, len(x.shape))))
        # (batch, time, ch) -> (time, batch, ch)
        embed, action, is_first = swap(embed), swap(action), swap(is_first)

        post, prior, pr_stats, po_stats = tools.static_scan(
            lambda prev_state, prev_act, embed, is_first: self.obs_step_stats(
        ...
        pr_stats = {k: swap(v) for k, v in pr_stats.items()}
        po_stats = {k: swap(v) for k, v in po_stats.items()}
        
        return post, prior, po_stats, pr_stats
    ...
    def observe(self, embed, action, is_first, state=None):
        ...
        # Return without detailed statistics
    ...
    def obs_step_stats(self, prev_state, prev_action, embed, is_first, ent_type=None):
        # initialize all prev_state
        if prev_state == None or torch.sum(is_first) == len(is_first):
            prev_state = self.initial(len(is_first))
            prev_action = torch.zeros(
        ...
        prior, im_stats = self.img_step_stats(prev_state, prev_action)
        x = torch.cat([prior["deter"], embed], -1)
        # (batch_size, prior_deter + embed) -> (batch_size, hidden)
        x = self._obs_out_layers(x)
        # (batch_size, hidden) -> (batch_size, stoch, discrete_num)
        ...
        post = {"stoch": stoch, "deter": prior["deter"], **stats}
        if ent_type=='dream':
            stats = self.get_dist(stats).entropy()
        return post, prior, im_stats, stats
    ...
    def img_step_stats(self, prev_state, prev_action):
        prev_stoch = prev_state["stoch"]
        if self._discrete:
            shape = list(prev_stoch.shape[:-2]) + [self._stoch * self._discrete]
            prev_stoch = prev_stoch.reshape(shape)
...
class MultiDecoder(nn.Module):
    def __init__(
        ...
        self.cnn_shapes = {
            k: v for k, v in shapes.items() if len(v) == 3 and any(re.match(pattern, k) for pattern in cnn_keys)
        }
        self.mlp_shapes = {
            k: v
            for k, v in shapes.items()
    ...
    def _make_image_dist(self, mean):
        ...
        if self._image_dist == "mae":
            return tools.MAEDist(mean)
...
class ConvDecoder(nn.Module):
    def __init__(
        ...
        self._linear_layer = nn.Linear(feat_size, int(out_ch))
