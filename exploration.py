...
from torchrl.data import DiscreteTensorSpec as CategoricalSpec
...
class PlanBehavior(nn.Module):
    def __init__(self, config, world_model, taskbehavior):
        """
        PPO-based planner with separate primary and secondary policies.
        ...
        action_spec_flo = CategoricalSpec(n=meta_action_quant, shape=(num_meta_action_lwr,), device=device,)

        self.actor_sec = nn.Sequential(
            nn.Linear(self.meta_policy_input_dim_lwr, self.num_cells, device=device),
            nn.Tanh(),
            nn.Linear(self.num_cells, self.num_cells, device=device),
            nn.Tanh(),
            nn.Linear(self.num_cells, num_meta_action_lwr * meta_action_quant, device=device),
            nn.Unflatten(dim=-1, unflattened_size=(num_meta_action_lwr, meta_action_quant)),
        )

        policy_module_sec = TensorDictModule(
            self.actor_sec, in_keys=["observation"], out_keys=["logits"]
        )

        self.policy_module_sec = ProbabilisticActor(
            module=policy_module_sec,
            spec=action_spec_flo,
            in_keys=["logits"],
            distribution_class=SummedCategorical,
            return_log_prob=True,
        )

        self.value_sec = nn.Sequential(
            nn.Linear(self.meta_policy_input_dim_lwr, self.num_cells, device=device),
            nn.Tanh(),
            nn.Linear(self.num_cells, self.num_cells, device=device),
            nn.Tanh(),
        ...
        self.value_module_sec = ValueOperator(
            module=self.value_sec,
            in_keys=["observation"],
            out_keys=["val_sec"],
        ).to('cuda')

        self.advantage_module_sec = GAE(
            gamma=self.gamma, 
            lmbda=self.lmbda, 
            value_network=self.value_module_sec, 
            average_gae=True,
        )
        
        self.advantage_module_sec.set_keys(
            advantage='adv_sec',
            value='val_sec',
            value_target='valt_sec',
        )

        self.loss_module_sec = ClipPPOLoss(
            actor_network=self.policy_module_sec,
            critic_network=self.value_module_sec,
            clip_epsilon=self.clip_epsilon,
            entropy_bonus=bool(self.entropy_eps),
            entropy_coef=self.entropy_eps,
            critic_coef=1.0,
        ...
        self.loss_module_sec.set_keys(
            advantage="adv_sec",
            value="val_sec",
            value_target="valt_sec",
            action="action_sec",
            sample_log_prob="sample_log_prob_sec",
        )
        ...
        self.replay_buffer_imag = ReplayBuffer(
            storage=LazyTensorStorage(max_size=self.buffer_size),
        )
        ...
        self.optim_v_sec = torch.optim.Adam(self.value_module_sec.parameters(), self.lr, eps=1e-07)
        self.optim_p_sec = torch.optim.Adam(self.policy_module_sec.parameters(), self.lr, eps=1e-07)
    ...
    def _add_to_buffer(
            self, 
            mode,
            observation = None, action = None, action_sec = None, 
            sample_log_prob_sec = None, sample_log_prob = None,
        ...
        Args:
            ...
            action_sec (Tensor): Optional secondary action taken.
            sample_log_prob (Tensor): Log-probability of primary action.
            sample_log_prob_sec (Tensor): Log-probability of secondary action.
        ...
        if mode == 'begin':
            ...
            self.real_buffer_dict = {
                ...
                'sample_log_prob_sec': sample_log_prob_sec,
            ...
            assert self.buffer_flag_b and self.buffer_flag_e, "Buffer end called without full fillment."
        ...
        self.process_ppo(
            {
                'key': 'implemented',                       
                'invert_key_flag': False,         
                'buffer': self.replay_buffer_imag,          
                'advantage': self.advantage_module_sec,                 
    ...
    def _train(self, logger):
        ...
        self.train_policy({
                'buffer': self.replay_buffer_imag,              # Buffer to sample from
                'loss_module': self.loss_module_sec,          # PPO loss module
                'optim_policy': self.optim_p_sec,
                'optim_value': self.optim_v_sec,
                'tag': '_r',                          # Logging tag (e.g. 'r', 'r_sec')
            },
            logger=logger)
